...
let retrievePlan = function (id, params) {
  return config.chargebee.plan.retrieve(id).request(function (error, result) {
    if (error) {
      return error;
    } else {
      return result;
...
let createPlan = function (data, params) {
  return config.chargebee.plan.create(data).request(function(error,result){
    if (error) {
      return error;
    } else {
      return result;
...
let updatePlan = function (id, data, params) {
  return config.chargebee.plan.update(id, data).request(function (error, result) {
    if (error) {
      return error;
    } else {
      return result;
...
let deletePlan = function (id, params) {
  return config.chargebee.plan.delete(id).request(
